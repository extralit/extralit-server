...
@pytest.fixture
def mock_observation_df(mock_schema_structure, schema_name="Observation"):
    df = pd.DataFrame({
...
@pytest.fixture
def mock_paper_extraction_true(mock_schema_structure, mock_observation_df):
    mock_data = {
...
@pytest.fixture
def mock_paper_extraction_pred(mock_schema_structure):
    df = pd.DataFrame({
        'Study_type': ['Hut trial', 'Lab based bioassay'],
        'Country': ['Country1', 'Country2'],
        'Site': ['Site1', 'Site2'],
